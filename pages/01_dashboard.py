...
try:
    ...
    st.error("⚠️ modules/ui_theme.py not found. Please verify the project folder structure.")
    st.stop()
...
if not top5:
    no_data_state(
        msg="No accounts analyzed yet.",
        hint="← Run an analysis from the main page to populate this ranking."
    )
else:
...
if current:
    ...
    with cc:
        ...
        <div class="kx-card kx-card-yellow">
            ...
            <div style="color:#6B7280;font-size:0.78rem;margin-top:0.5rem;">{years} yr(s) without purchase</div>
        </div>""", unsafe_allow_html=True)
...
if current:
    try:
        ...
        with col_info:
            ...
            <div style="padding:0.6rem 1rem;border:1px solid #1E2A35;border-radius:6px;
                ...
                &nbsp;|&nbsp; <span style="color:#00FFB2;">100% locally generated</span>
            </div>
            """, unsafe_allow_html=True)
    ...
    <div style="color:#6B7280;font-family:'Share Tech Mono',monospace;font-size:0.82rem;
                padding:0.6rem 0;">
        Run an analysis first to enable report export.
    </div>
    """, unsafe_allow_html=True)
