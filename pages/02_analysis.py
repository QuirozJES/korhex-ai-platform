...
page_header(
    ...
    subtitle="6 B2B intelligence elements generated locally by Llama 3 via Ollama — no customer data sent to the cloud."
)
...
if not current:
    no_data_state(
        ...
        hint="← Enter a company in the main page sidebar and click 'Execute Analysis'."
    )
    st.stop()
...
<div class="kx-card kx-card-accent" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:0.5rem;">
    <div>
        ...
        <div style="color:#6B7280;font-size:0.8rem;font-family:'Share Tech Mono',monospace;">
            {industry} &nbsp;|&nbsp; {years} year(s) inactive
        </div>
    </div>
...
st.markdown("---")
st.markdown('<div class="kx-section-title">◈ 6 Account Intelligence Building Blocks</div>', unsafe_allow_html=True)
    ...
    with st.expander(f"{icon} {title}", expanded=(idx <= 3)):
        if section_text:
            ...
            <div class="kx-card {card_class}" style="margin-bottom:0.8rem;">
                ...
                <div style="font-size:0.9rem;line-height:1.7;color:#C9D1D9;white-space:pre-wrap;">
            """, unsafe_allow_html=True)
            st.write_stream(stream_data(section_text))
            st.markdown("""
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            <div style="color:#6B7280;font-size:0.85rem;font-family:'Share Tech Mono',monospace;padding:1rem 0;">
                No data is available for this section. The scraper did not find information for '{web_key}'.
            </div>
            """, unsafe_allow_html=True)
    ...
    <div style="display:flex;align-items:center;gap:0.8rem;">
        ...
        <div>
            <div style="font-weight:700;color:#FFF;">
                {"Analysis verified — no hallucinations detected" if audit_ok else "Potential inaccuracies detected"}
            </div>
