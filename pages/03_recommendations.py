...
<div class="kx-card kx-card-accent" style="display:flex;justify-content:space-between;align-items:center;">
    ...
    <div>
        <span class="kx-badge badge-green">{len(products)} MATCHED SOLUTIONS</span>
    </div>
</div>
...
if not products:
    no_data_state(
        msg="No recommended products were found.",
        hint="Verify that modules/rag.py and data/portfolio.json are correctly configured."
    )
    st.stop()

# ── Product grid ───────────────────────────────────────────────────────────────
st.markdown('<div class="kx-section-title">◈ Solutions Recommended by RAG</div>', unsafe_allow_html=True)
...
for i, product in enumerate(products):
    ...
    with col_side:
        ...
        if pain_points:
            st.markdown('<div class="kx-section-title" style="margin-top:0.8rem;">◈ Covered Pain Points</div>', unsafe_allow_html=True)
...
st.markdown("---")
st.markdown('<div class="kx-section-title">◈ Customer Pain → Proposed Solution</div>', unsafe_allow_html=True)
...
if pains and products:
    ...
    st.info("Run the full analysis to view the pain-to-solution mapping.")
