...
# Importamos utilidades del tema
...
def stream_data(text: str):
    """Generator that emits text chunks with a small delay for the typewriter effect."""
    if not text:
        return
...
# Extraemos ms asegurando que sea un número
raw_ms      = analysis.get("processing_ms", 0)
try:
    ms_val = float(raw_ms)
    gen_time_display = f"{ms_val / 1000:.1f}s" if ms_val > 0 else "—"
except:
    gen_time_display = "—"
...
audit_label = "AUDIT PASSED — Content verified by Compliance Agent" if audit_ok \
              else "AUDIT FLAG — Review content before use"
...
with m2: st.metric("Products cited", len(products))
with m3: st.metric("Years inactive", years)
# FIX: Ahora usa la variable formateada gen_time_display
with m4: st.metric("Generation time", gen_time_display)
